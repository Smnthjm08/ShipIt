...
- Command injection in build-in-container.ts — shell interpolation of user input
- Path traversal via outputDir — no sanitization before path.join
- GitHub OAuth token embedded in git clone URL
- No input validation on any API endpoint
- Hardcoded localhost:8001 in deployment-table.tsx
...
- S3 cleanup on deployment/project deletion
...
proxy/src/index.ts — split lookup from serving. The route handler in index.ts does subdomain parsing, database lookup, S3 key construction, and stream piping in one function. Extract resolveDeploymentFromSubdomain(host) → returns { deploymentId, prefix } and streamS3File(key, res) as separate functions.

@repo/shared Redis — eliminate the any exports. The redis/client.ts exports RedisClient = any. Replace with import type { RedisClientType } from 'redis' and type the exports properly. This propagates type safety to shipyard and backend with zero behavior change.

new-project.controller.ts — split into two files. The file contains two unrelated exported controllers: GitHub repo search and project creation. They share no logic. Split into search-repos.controller.ts and create-project.controller.ts.
...
- Fix the hardcoded localhost:8001 deployment URL
...
- Add dead-letter queue / stale-deployment recovery job
...
- Add deployment concurrency limits per user (prevent one user monopolizing all workers)
...
The apps/ws-server app is a stub with a one-line body. Either remove it or add a // TODO issue reference so contributors understand its status. Stubs that look like real services confuse new contributors.

Naming

apps/shipyard/src/build-in-container.ts — the function exported is buildInContainer, which matches the file name, but the file also owns S3 upload logic. Rename to run-build.ts or split.

The new/ route and new-project.controller.ts naming is vague. Rename to projects/new/ (web) and create-project.controller.ts (backend) to match REST conventions.

Documentation

The README.md should include a one-command local setup (docker compose up), a diagram of the deployment pipeline, and the env var table currently only in CLAUDE.md. Architecture diagrams are in docs/ but there is no link to them from the README.

Add a CONTRIBUTING.md with the branch convention, PR process, and the command to run before pushing (pnpm lint && pnpm check-types).
...
Add GET /health to shipyard and proxy (backend already has one). Health checks should return Redis connectivity, DB connectivity, and Docker daemon status from shipyard. Without these, a load balancer or orchestrator cannot determine whether a worker is healthy.

Testing

Start with integration tests over unit tests for this architecture. The most valuable test is: given a mock Git repo and a local Docker daemon, does a full build complete and produce S3 artifacts? This tests every layer except S3 (which can use LocalStack or MinIO in CI).

Security tooling

Add pnpm audit to the pre-commit hook alongside format. Add gitleaks or trufflesecurity/trufflehog to CI to prevent accidental secret commits.
...
- No CI to give them feedback on a PR
- A security vulnerability in build-in-container.ts that they might accidentally exploit while testing
...
1. One-command setup. docker compose up starts Postgres, Redis, and MinIO. pnpm dev starts all apps. The README should show this in the first code block.
2. A working demo. Either a live demo link or a docker compose --profile demo up that seeds sample data and runs the full platform locally.
3. Issue templates. .github/ISSUE_TEMPLATE/bug_report.md and feature_request.md. Signals that contributions are expected and organized.
4. PR template. .github/pull_request_template.md with checkboxes: does it pass lint, does it pass type-check, is there a test.
...
6. Architecture diagram in the README. The docs/ directory exists but is not linked. The pipeline diagram should be in the README, above the fold.
7. Fix the security issues before public release. Command injection in a user-facing deployment tool is not an acceptable known issue for an open-source project. It will be found, reported loudly, and become the project's identity.
