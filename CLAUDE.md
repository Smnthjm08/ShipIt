...
| `ws-server` | —    | WebSocket server (stub, not yet implemented) |
| `proxy`     | —    | Subdomain → S3 asset server                  |
...
- **`@repo/shared`** — Redis client factory and typed queue/pub-sub helpers. `redisQueue` is the build job queue. Always use `connectRedis`/`disconnectRedis` wrappers rather than the raw client.
...
- The `ws-server` app is a stub. Don't wire real WebSocket logic there without scaffolding it properly first.
